...
let getObservableFromEventTarget;
...
class EventTarget {
  ...
  on(event, options = {}) {
    if (!isEventTarget(this))
      throw new ERR_INVALID_THIS('EventTarget');
    getObservableFromEventTarget ??= require('internal/observable');
    return getObservableFromEventTarget(event, options, new SafeWeakRef(this));
  }
