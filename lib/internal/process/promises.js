...
function hasAsyncContextFrame() {
  hasAsyncContextFrame_ ??= require('internal/options')
    .getOptionValue('--async-context-frame');
  return hasAsyncContextFrame_;
}
...
function unhandledRejection(promise, reason) {
  ...
  maybeUnhandledPromises.set(promise, {
    ...
    emit,
    contextFrame: hasAsyncContextFrame() && AsyncContextFrame.current(),
  });
...
function processPromiseRejections() {
  ...
  while (len--) {
    ...
    const { reason, uid, emit, contextFrame } = promiseInfo;
    ...
    if (hasAsyncContextFrame()) {
      priorContextFrame = AsyncContextFrame.exchange(contextFrame);
    }
    try {
      ...
      if (hasAsyncContextFrame()) {
        AsyncContextFrame.exchange(priorContextFrame);
      }
      if (needPop) {
        if (typeof promiseAsyncId !== 'undefined') {
