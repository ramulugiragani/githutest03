...
class Http2Stream extends Duplex {
  ...
  _destroy(err, callback) {
    ...
    let sessionCode;
    if (sessionState.goawayCode === NGHTTP2_NO_ERROR) {
      // Received NO_ERROR (GOAWAY) from remote peer
      sessionCode = NGHTTP2_INTERNAL_ERROR;
    } else if (sessionState.destroyCode === NGHTTP2_NO_ERROR) {
      // Code to send to remote peer
      sessionCode = isServerSession ? NGHTTP2_INTERNAL_ERROR : NGHTTP2_CANCEL;
    } else {
      sessionCode = sessionState.goawayCode ?? sessionState.destroyCode;
    }
    ...
    if (!this.closed) {
      ...
      closeStream(this, code ?? defaultCode, rstStreamStatus);
    }
    ...
    // The client should not throw when it sends CANCEL to the server
    // before ending the request.
    // The client should throw when it sends CANCEL to the server
    // after ending the request.
    // The client should throw when it receives CANCEL from the server.
    // The server should not throw when it receives CANCEL from the client.
    // The first check is not sufficient as it doesn't cover cases
    // with connection resets like test-http2-max-session-memory.js
    const throwOnCancel = !this.aborted && !isServerSession;

    if (err == null &&
        code !== NGHTTP2_NO_ERROR &&
        (code !== NGHTTP2_CANCEL || throwOnCancel)) {
...
function socketOnClose() {
  ...
  if (session !== undefined) {
    ...
    const state = session[kState];
    const code = session[kType] === NGHTTP2_SESSION_SERVER ?
      NGHTTP2_CANCEL :
      NGHTTP2_INTERNAL_ERROR;
