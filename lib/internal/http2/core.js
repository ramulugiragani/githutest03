...
function onFrameError(id, type, code) {
  ...
  if (stream) {
    stream.emit('frameError', type, code, id);
    stream[kState].errorSendingFrame = true;
    stream[kState].rstCode = code;
    stream.destroy(new ERR_HTTP2_STREAM_ERROR(nameForErrorCode[code] || code));
  } else {
    session[kState].destroyCode = code;
    session.destroy(new ERR_HTTP2_SESSION_ERROR(nameForErrorCode[code] || code));
  }
}
...
function onGoawayData(code, lastStreamID, buf) {
  ...
  if (code === NGHTTP2_NO_ERROR) {
    ...
    closeSession(
      session,
      code,
      new ERR_HTTP2_SESSION_ERROR(nameForErrorCode[code] || code),
      true,
    );
  }
}
...
function closeSession(session, code, error, closed) {
  ...
  if (handle !== undefined) {
    handle.ondone = FunctionPrototypeBind(finishSessionClose,
                                          null, session, error);
    handle.destroy(code, closed ?? socket.destroyed);
  } else {
...
class Http2Session extends EventEmitter {
  ...
  // FIXME(szmarczak): goaway without connection close
  // for error codes other than NO_ERROR is against the spec.
  // See https://datatracker.ietf.org/doc/html/rfc9113#section-5.4.1
  ...
  close(callback) {
    ...
    state.flags |= SESSION_FLAGS_CLOSED;
    if (typeof callback === 'function')
      this.once('close', callback);

    if (state.streams.size > 0 || state.pendingStreams.size > 0) {
      this.goaway();
    } else {
      this.destroy();
    }
...
class Http2Stream extends Duplex {
  ...
  _destroy(err, callback) {
    ...
    const isServerSession = session[kType] === NGHTTP2_SESSION_SERVER;

    debugStream(id || 'pending', session[kType], 'destroying stream');

    // Do not use this.headersSent instead of trailersReady.
    // headersSent is true when it **tries** to send headers.
    // It is true even though it fails.
    if (!this.closed &&
        (!this[kState].errorSendingFrame || this[kState].trailersReady || isServerSession)) {
      ...
      if (destroyCode === NGHTTP2_NO_ERROR) {
        // Code to send to remote peer
        destroyCode = isServerSession ? NGHTTP2_INTERNAL_ERROR : NGHTTP2_CANCEL;
      }

      const sessionCode = goawayCode ?? destroyCode;
      let code = this.rstCode ?? sessionCode;

      if (err != null && code === null) {
        code = err instanceof AbortError ? NGHTTP2_CANCEL : NGHTTP2_INTERNAL_ERROR;
      }

      const connect = this[kSentHeaders]?.[HTTP2_HEADER_METHOD] === HTTP2_METHOD_CONNECT;
      if (code === null && connect) {
        code = NGHTTP2_CONNECT_ERROR;
      }

      const defaultCode = isServerSession ? NGHTTP2_INTERNAL_ERROR : NGHTTP2_CANCEL;
      code ??= defaultCode;

      const rstStreamStatus = hasHandle ? kForceRstStream : kNoRstStream;
      closeStream(this, code, rstStreamStatus);
    }
    ...
    sessionState.writeQueueSize -= this[kState].writeQueueSize;
    this[kState].writeQueueSize = 0;
