...
const builtInObjects = new SafeSet(
  ArrayPrototypeFilter(
    ...
    (e) => SafeRegExpPrototypeTest(/^[A-Z][a-zA-Z0-9]+$/, e)
  )
);
...
function strEscape(str) {
  ...
  if (str.length < 5000 && !SafeRegExpPrototypeTest(escapeTest, str))
    return addQuotes(str, singleQuote);
  if (str.length > 100) {
    str = SafeRegExpPrototypeSymbolReplace(escapeReplace, str, escapeFn);
    return addQuotes(str, singleQuote);
  }
...
function formatError(err, constructor, tag, ctx, keys) {
  ...
  if (!ctx.showHidden && keys.length !== 0) {
    ArrayPrototypeForEach(['name', 'message', 'stack'], (name) => {
  ...
  if (constructor === null ||
    ...
    const prefix = StringPrototypeSlice(
      getPrefix(constructor, tag, fallback), 0, -1);
    if (name !== prefix) {
  ...
  if (stackStart === -1) {
    ...
    let previousIndex = stackStart + 1;
    let index;
    do {
      index = StringPrototypeIndexOf(stack, '\n', previousIndex);
      if (index === -1) index = undefined;
      const line = StringPrototypeSlice(stack, previousIndex, index);
      ...
      if (core !== null && NativeModule.exists(core[1])) {
        ...
        newStack += '\n';
        let pos = 0;
        while (nodeModule = RegExpPrototypeExec(nodeModulesRegExp, line)) {
          // '/node_modules/'.length === 14
          newStack += StringPrototypeSlice(line, pos, nodeModule.index + 14);
          newStack += ctx.stylize(nodeModule[1], 'module');
          pos = nodeModule.index + nodeModule[0].length;
        }
        newStack += pos === 0 ? line : StringPrototypeSlice(line, pos);
      }
  ...
  if (ctx.indentationLvl !== 0) {
    ...
    stack = SafeStringPrototypeReplaceAll(stack, '\n', `\n${indentation}`);
  }
  return stack;
}
...
function formatArrayBuffer(ctx, value) {
  ...
  let str = StringPrototypeTrim(SafeRegExpPrototypeSymbolReplace(
    /(.{2})/g,
...
function formatProperty(ctx, value, recurseTimes, key, type, desc,
  ...
  if (typeof key === 'symbol') {
    const tmp = SafeRegExpPrototypeSymbolReplace(
    ...
    const tmp = SafeRegExpPrototypeSymbolReplace(strEscapeSequencesReplacer,
                                                 key, escapeFn);
    name = `[${tmp}]`;
  } else if (SafeRegExpPrototypeTest(keyStrRegExp, key)) {
...
const firstErrorLine = (error) =>
  SafeStringPrototypeSplit(error.message, '\n', 1)[0];
